...
def calculation_overx():
    global calculation
    try:
        calculation = str(eval("1/" + calculation))
        text_result.delete(1.0, "end")
        text_result.insert(1.0, calculation)
    except:
        clear_field()
...
btn_open_bracket.grid(row=2, column=2)

btn_close_bracket = tk.Button(root, text=")", command=lambda: add_to_calculation(")"), width=5, font=("Arial", 14))
btn_close_bracket.grid(row=2, column=3)

btn_backspace = tk.Button(root, text="<-", command=lambda: backspace(), width=5, font=("Arial", 14))
btn_backspace.grid(row=2, column=4)

btn_power = tk.Button(root, text="^", command=lambda: add_to_calculation("**"), width=5, font=("Arial", 14))
btn_power.grid(row=3, column=2)

btn_one_over = tk.Button(root, text="1/x", command=lambda: calculation_overx(), width=5, font=("Arial", 14))
btn_one_over.grid(row=3, column=1)

btn_percentage = tk.Button(root, text="%", command=lambda: percentage(), width=5, font=("Arial", 14))
...
btn_enter = tk.Button(root, text="=", command=lambda: evaluate_calculation(), width=5, font=("Arial", 14))
btn_enter.grid(row=7, column=4)
